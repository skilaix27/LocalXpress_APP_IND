...
export async function createOrderInCentralApi(
  ...
  const payload = {
    order_code:                 order.order_code,   // LXP-D... generated by this app
  ...
  // ── Debug log (remove once central API integration is confirmed) ─────────────
  console.log("[centralApi] Sending payload to central backend:", {
  ...
  if (!response.ok) {
    console.error("[centralApi] Error response:", response.status, data.error ?? "(no message)");
  ...
  console.log(
    `[centralApi] Order created — id: ${data.id ?? "unknown"}, code: ${centralCode}, local: ${order.order_code}`
  );
  return {
    ok:                  true,
    duplicate:           false,
